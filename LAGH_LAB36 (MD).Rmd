...
Clúster por genes (grupos de genes similares) se encuentra en los renglones. 

Paso 11. Eliminar dendograma de las columnas (pacientes).
```{r}
pheatmap(genes, fontsize = 6, cluster_rows = T, cluster_cols = F)

```

Clúster por pacientes (grupos de pacientes similares) se encuentra en las columnas.

Paso 12. Eliminar dendograma de los renglones (genes).
```{r}
...
```{r}
pheatmap(sub, fontsize = 6, cluster_rows = T, cluster_cols = T, annotation_row = annotation_row, 
         annotation_col = annotation_col, treeheight_row = 0, treeheight_col = 0, 
         main = "Expresión Genética")
